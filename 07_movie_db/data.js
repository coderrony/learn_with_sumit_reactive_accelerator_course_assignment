...
export const getMovieById = (id) => {
  const movie = movieList.results.find((movie) => movie.id === Number(id));
...
export const updateMovie = (id, data) => {
  ...
  // Iterate through the movie list to find the movie with the given ID
  movieList.results.forEach((movie) => {
    if (movie.id === Number(id)) {
      // If the movie is found, update its title
      movie.title = data.title;
      saveMovie = movie; // Save the updated movie
    }
  });

  // If a movie was found and updated, return the updated movie; otherwise, return null
  return saveMovie;
};
export const deleteMovie = (id) => {
  const index = movieList.results.findIndex((item) => item.id === Number(id));
